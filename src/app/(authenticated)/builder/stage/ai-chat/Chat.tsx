...
import { Send, Bot, User, Terminal, Loader2, Circle, Clock, Sparkles, ArrowRight } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';

export default function Chat() {
    const { stompClient, isConnected } = useStomp();
    const [chatId, setChatId] = useState('bf1457d9-9f20-428c-90ea-bc3ff6893d29');
            ...
            if (token.startsWith("Error:")) {
                ...
                setLogs((prev) => {
                    ...
                    if (lastLog && lastLog.type === 'ai') {
                        ...
                        // ПРОВЕРКА НА КЛЮЧЕВОЕ СЛОВО В НАКОПЛЕННОМ ТЕКСТЕ
                        if (updatedText.includes("INTERVIEW_COMPLETED")) {
                            setIsCompleted(true);
                            // Убираем технический тег из видимого текста
    ...
    return (
        <div className="px-4 font-sans">
            <div className="w-full max-w-2xl h-[700px] flex flex-col overflow-hidden">
                ...
                {/* HEADER */}
                <header className="bg-white border-b border-slate-100 px-5 py-3.5 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1.5">
                                <Circle size={8} className={`${isConnected ? 'fill-green-500 text-green-500 animate-pulse' : 'fill-red-500 text-red-500'}`} />
                ...
                <main className="flex-1 overflow-y-auto p-5 bg-slate-50/50 space-y-4">
                    {logs.map((log, i) => (
                        <div key={i} className={`flex ${log.type === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in duration-300`}>
                            <div className={`flex gap-2.5 max-w-[90%] ${log.type === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    {isThinking && (
                        <div className="flex justify-start animate-pulse">
                            <div className="flex gap-2.5 items-center bg-white border border-slate-200 px-4 py-2 rounded-2xl rounded-tl-none shadow-sm">
                                <div className="flex gap-1">
                                    <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce" />
                                    <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce [animation-delay:0.2s]" />
                                    <div className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-bounce [animation-delay:0.4s]" />
                                </div>
                                <span className="text-[11px] font-mono text-slate-400 ml-2">{thinkingTime.toFixed(1)}s</span>
                            </div>
                        </div>
                    )}
                    ...
                    {/* COMPLETION MESSAGE */}
                    {isCompleted && (
                        <div className="p-4 bg-green-50 border border-green-100 rounded-2xl text-center space-y-2 animate-in zoom-in-95">
                            ...
                            <p className="text-xs text-green-600">Нажмите кнопку ниже, чтобы перейти к финальному этапу.</p>
                        </div>
                    )}
                ...
                <footer className="p-4 bg-white border-t border-slate-100">
                    {!isCompleted ? (
                        <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-2xl p-1.5 focus-within:ring-2 focus-within:ring-indigo-500/10 transition-all">
                            <input
                        ...
                        <button
                            onClick={() => window.location.href = '/builder/stage/generation'}
