...
export default function SetPasswordForm() {
    ...
    const mutation = useMutation({
        mutationFn: async (data: FormValues) => {
            return axiosWithToken.post("/settings/password", {
    ...
    return (
        <Card className="rounded-[2.5rem] shadow-xl border border-zinc-100 bg-zinc-50/50 p-8 hover:bg-white hover:border-black hover:shadow-xl transition-all duration-300">
            <CardHeader>
                ...
                <CardDescription>Пожалуйста, создайте надежный пароль.</CardDescription>
            </CardHeader>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
                <CardContent className="space-y-4">
                    ...
                    {mutation.isError && (
                        <div className="text-sm text-red-600">
                            {(mutation.error as any)?.response?.data?.message || "Ошибка при установке пароля"}
                        </div>
                    )}

                    {mutation.isSuccess && (
                        <div className="text-sm text-green-600">Пароль успешно установлен</div>
                    )}
                </CardContent>
                ...
                <CardFooter className="border-t px-6 py-4 flex justify-between">
                    ...
                    >
                        {mutation.isPending ? "Установка..." : "Установить пароль"}
                    </Button>
                </CardFooter>
            </form>
        </Card>
